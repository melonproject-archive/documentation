...
The constructor for this contract requires the parameters `_delay` and `_postDeployOwner`. The former specifies the number of seconds, setting the `THAWING_DELAY` state variable to this value. The latter is an address specifying the owner of the contract after it is deployed. The `lastThaw` state variable is set to the current `block.timestamp` value.
&nbsp;
